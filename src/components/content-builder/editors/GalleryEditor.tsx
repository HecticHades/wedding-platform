...
export function GalleryEditor({ initialContent }: GalleryEditorProps) {
  // Type guard to ensure we have gallery content
  const typedContent =
    initialContent.type === "gallery"
      ? initialContent
      : { type: "gallery" as const, title: "Photo Gallery", photos: [] };

  const [title, setTitle] = useState(typedContent.title);
  const [photos, setPhotos] = useState<Photo[]>(typedContent.photos);
        ...
        {photos.length > 0 ? (
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-1">
              Upload your engagement photos or favorite memories together.
            </p>
          </div>
        )}
      </div>
